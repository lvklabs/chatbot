...
template<typename T=int> class aligner{
...
public:
  ...
  struct alin {
    ...
    char* seg; // Segment of b[] with the letters
    char* a;   // Alignment of a
    char* b;   // Alignment of b
    ...
    alin(T _score, int _begin, int _end, int _beginW, int _endW, int _substitutions, int _inserts, int _deletions, char* _a, char* _b) :
   ...
   */
  alin* align(const char* a, const int tj, const char* b, const int ti, const int mode = SEMILOCAL){
    ...
    */
    ...
    char* answerA = new char[tj+ti];
    char* answerB = new char[tj+ti];
    int pA = 0;
    int pB = 0;
    ...
    //þ Decide the number of word that each character belongs to
    int nwords=0;
    for(int i=0; i<ti; i++){ 
      if( b[i] == ' ' || b[i] == '_' ){
	// word change at _i_
    ...
    */
    ...
    for(j=1;j<=tj;j++){
	m[j*W+1] = max(  m[W*(j-1)]+sc->dSub(a[j-1],b[0]) , m[W*(j-1)+1]+sc->dSkip(a[j-1]) );
	if(a[j-1]==' '|| a[j-1]=='_' ){spacesA++;}
	if(score < m[W*j+1]){
	  score = m[W*j+1];
	  bestJ = j;
	  bestI = 0;
    ...
    for(i=1;i<=ti;i++){
	m[ W+i ] = max(  m[ i-1 ]+sc->dSub(a[0],b[i-1])  ,  m[ W+i-1 ]+sc->dSkip(b[i-1])  );
	if(score < m[W+i]){
	  score = m[W+i];
	  bestJ = 0;
	  bestI = i;
	...
	m[W*j+i] = max(threshold,max(i1,max(i1,max(i2,max(i3,max(i4,i5))))));
    ...
    */
    ...
    // Print the alignment
    char* newA = new char[pA+1];
    char* newB = new char[pB+1];
    newA[pA]=0;
    newB[pB]=0;
    --pA;
    --pB;
